...
`mdtool` supports Word's built-in citation and bibliography system for IEEE-style referencing.

### togit direction (`.docx` → `.md`)

When a document uses Word's **Insert Citation** feature (References tab → Citations & Bibliography group), `togit` converts structured citations to Markdown intra-document hyperlinks and renders the bibliography section as plain text.

**In-text citations** are converted as follows. The `[` and `]` bracket characters are kept as literal text; the content between them becomes a Markdown hyperlink to the corresponding entry in the references section:

- Single citation: `[[1]](#ref-1)` — renders as `[`[1](#ref-1)`]`
- Single citation with page: `[[1, p. 5]](#ref-1)` — renders as `[`[1, p. 5](#ref-1)`]`
- Multiple citations in one bracket: `[[1](#ref-1), [2](#ref-2)]` — renders as `[`[1](#ref-1)`, `[2](#ref-2)`]`
- Multiple citations, last with page: `[[1](#ref-1), [2, p. 8](#ref-2)]`

**The bibliography section** (inserted via References → Bibliography) is rendered using the heading level and text from the Word document, followed by each reference entry as plain text (all character formatting such as italics and bold is stripped). Each entry is prefixed with an HTML anchor that the in-text hyperlinks point to:

```
## References

<a id="ref-1"></a>[1] N. Nguyen, "Title," Journal Name, vol. 1, pp. 1–10, 2020.
<a id="ref-2"></a>[2] A. Smith, Book Title. Publisher, 2019.
```

**Style enforcement:** `togit` reads the bibliography style from `word/settings.xml`. If the style is set to anything other than IEEE, `togit` fails immediately with an error message. Change the style in Word (References → Style → IEEE) and update the bibliography before re-running `togit`.

**Manually typed references** (text that looks like a reference list but was not inserted via Word's citation tools) are treated as normal paragraphs and receive no special treatment.

### fromgit direction (`.md` → `.docx`)

`fromgit` does **not** reconstruct Word citations from Markdown. Hyperlink-annotated citation text produced by `togit` round-trips as plain Markdown text — the `[`[1](#ref-1)`]` syntax is reproduced as a regular hyperlink in the `.docx`, not as a structured Word citation field. The bibliography section is reproduced as plain paragraphs with no connection to Word's citation database.

If you need to add or modify citations in a document, do so in Word directly using the Insert Citation workflow, then run `togit` again.
...
- **Citations: togit only.** Word structured citations (Insert Citation) and bibliography sections are converted to Markdown hyperlinks during `togit`. The reverse (`fromgit`) does not reconstruct Word citation fields; the Markdown hyperlink text is reproduced as plain hyperlinks in the `.docx`. If you need to edit citations, do so in Word and re-run `togit`.
- **Citations: IEEE only.** Only IEEE bibliography style is supported. Documents using any other style (APA, MLA, Chicago, etc.) will be rejected by `togit` with an error. Change the style in Word and update the bibliography before syncing.
- **Citations: manually typed references** (text entered without using Insert Citation) are treated as normal paragraphs and receive no anchor or hyperlink treatment.

## Changelog

**1.7** *(15 Jun 2026)*
- `togit` now converts Word structured citations (Insert Citation) and the bibliography section (References → Bibliography) to Markdown. Only IEEE style is supported; any other bibliography style detected in `word/settings.xml` causes the sync to fail with an informative error.
- In-text citations become intra-document hyperlinks. A single citation `[1]` renders as `[[1]](#ref-1)`; adjacent citations `[1, 2]` render as `[[1](#ref-1), [2](#ref-2)]`; a citation with a page annotation `[1, p. 5]` renders as `[[1, p. 5](#ref-1)]`. The `[` and `]` bracket characters are preserved as literal text flanking the hyperlink.
- The bibliography section is rendered using the heading level from Word followed by each reference entry as flat plain text (character formatting stripped), with an HTML anchor `<a id="ref-N"></a>` prepended to each entry.
- Manually typed references (not inserted via Word's citation tools) are treated as normal paragraphs.
- `fromgit` does not reconstruct Word citations from Markdown. See [Citations and references](#citations-and-references) and [Limitations](#limitations).
